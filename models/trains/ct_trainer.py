...
class CtTrainer(object):
    ...
    def run_epoch(self, phase, epoch, data_loader):
        ...
        for iter_id, batch in enumerate(data_loader):
            ...
            if opt.test and phase == "val":
                self.save_result(output, batch, results)
                # print("current itera results is", results)
    ...
    def save_result(self, output, batch, results):
        ...
        dets = dets.detach().cpu().numpy().reshape(1, -1, dets.shape[2])
        dets_out = ctdet_post_process(dets.copy(), batch['meta']['c'].cpu().numpy(), batch['meta']['s'].cpu().numpy(),
    ...
    # def val_voc(self, result_json, ann):
    #     return self.run_epoch_voc( result_json)

    def run_epoch_voc(self, result_json, anno_json_pth, score_th, class_num):
        from models.eval_utils.calc_map import calc_voc_map
        return calc_voc_map(result_json, anno_json_pth, score_th, class_num)
    # def save_result_voc(self, output, batch, results):
    #     reg = output['reg'] if self.opt.reg_offset else None
    #     dets = ctdet_decode(output['hm'], output['wh'], reg=reg, cat_spec_wh=self.opt.cat_spec_wh, K=self.opt.K)
    #     dets = dets.detach().cpu().numpy().reshape(1, -1, dets.shape[2])
    #     dets_out = ctdet_post_process(dets.copy(), batch['meta']['c'].cpu().numpy(), batch['meta']['s'].cpu().numpy(),
    #                                   output['hm'].shape[2], output['hm'].shape[3], output['hm'].shape[1])
    #     # print("dets_out is", dets_out)
    #     results[batch['meta']['img_id'].cpu().numpy()[0]] = dets_out[0]
    #     print("batch shape is", batch['meta']['img_id'].cpu().numpy()[0])
    #     print("dets_out[0] is", dets_out[0])
